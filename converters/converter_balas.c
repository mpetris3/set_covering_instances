/*
・balasタイプのインスタンスをscpタイプのフォーマットに変換するプログラムです。
・コンパイルの方法は、

     % gcc -O2 -o balas2scp balas2scp.c

  です。実行ファイル 'balas2scp' ができます。
・使い方は、

     % gunzip -c bus1.inp.gz | ./balas2scp
...
void readscp(void) {
  ...
  /*** 問題の読み込み ***/
  for(j=0;j<n;j++) {fscanf(fp,"%d",&cost[j]);}   /*** 重みの読み込み ***/
